...
# UI shell — Salesforce/Tableau 스타일 클린 SaaS
from branding import (
    inject_brand_css, render_app_header, render_kpi_cards,
    render_landing_page, render_loading_progress,
...
def _display_top_kpi(data):
    ...
    # IRR 추세 인디케이터: 할인율 5% 대비
    lcoe_val = base.get('lcoe_krw_per_mwh', 0)
    lcoe_usd = lcoe_val / 1350 if lcoe_val else 0  # ₩/MWh → $/MWh
    sim_hours = max(len(pv_power), 1)
    co2_annual = co2_avoided * (8760 / sim_hours) if sim_hours > 0 else 0
    irr_pct = base.get('irr_pct', 0)
    payback = base.get('payback_years', 0)
    
    irr_dir = "positive" if irr_pct >= 5.0 else ("negative" if irr_pct < 0 else "neutral")
    irr_delta = f"{'+' if irr_pct >= 5 else ''}{irr_pct - 5.0:.1f}pp vs. discount rate"
    ss_dir = "positive" if self_sufficiency >= 50 else "neutral"
    ...
    render_kpi_cards([
        ...
        {
            "label": "CO₂ avoided",
            "value": f"{co2_annual:,.0f}",
            "unit": "tCO₂/yr",
            "delta": "Annualized vs. grid baseline",
            "delta_dir": "positive",
        },
        {
            "label": "PUE",
            "value": f"{pue:.2f}",
            "delta": "Power usage effectiveness",
            "delta_dir": "neutral",
        },
        {
