...
def inject_brand_css():
    ...
    # PWA meta tags injection (iOS "Add to Home Screen" 지원)
    pwa_script = """
    <script>
    (function() {
        // iOS PWA / Apple-specific meta tags
        var meta_tags = [
            {name: 'apple-mobile-web-app-capable', content: 'yes'},
            {name: 'apple-mobile-web-app-status-bar-style', content: 'black-translucent'},
            {name: 'apple-mobile-web-app-title', content: 'IES NRL'},
            {name: 'theme-color', content: '#0B5CAB'}
        ];
        meta_tags.forEach(function(tag) {
            if (!document.querySelector('meta[name="' + tag.name + '"]')) {
                var m = document.createElement('meta');
                m.setAttribute('name', tag.name);
                m.setAttribute('content', tag.content);
                document.head.appendChild(m);
            }
        });

        // Apple touch icon (홈 화면 추가 시 아이콘)
        if (!document.querySelector('link[rel="apple-touch-icon"]')) {
            var icon = document.createElement('link');
            icon.setAttribute('rel', 'apple-touch-icon');
            icon.setAttribute('href', 'https://raw.githubusercontent.com/sjoonkwon0531/CESM_DT5/main/icon_512.png');
        ...
        if (!document.querySelector('link[rel="manifest"]')) {
            ...
            manifest.setAttribute('href', 'https://raw.githubusercontent.com/sjoonkwon0531/CESM_DT5/main/manifest.json');
...
def render_app_header():
    ...
    html = (
        ...
        f"<div class='brand-header-left'>"
        f"<div class='brand-logo'>{logo_svg}</div>"
        f"<div>"
        f"<div class='brand-title'>{BRAND['product_name']}</div>"
        f"<div class='brand-subtitle'>{BRAND['product_subtitle']}</div>"
        f"</div>"
        f"</div>"
        f"<div class='brand-header-right'>"
        f"<span><span class='brand-status-dot'></span>System operational</span>"
        f"<span class='brand-version-pill'>{BRAND['product_version']}</span>"
        f"</div>"
        f"</div>"
    )
...
def render_kpi_cards(kpis):
    ...
    for k in kpis:
        ...
        # 한 줄로 만들어 Markdown 코드블록 오해 방지
        cards_html += (
            f"<div class='kpi-card'>"
            f"<div class='kpi-label'>{k['label']}</div>"
            f"<div class='kpi-value'>{k['value']}{unit}</div>"
            f"{delta}"
            f"</div>"
        )
...
def render_landing_page():
    ...
    for f in features:
        icon_svg = (
            ...
            f'stroke-linejoin="round"><path d="{f["icon"]}"/></svg>'
        )
        cards += (
            f"<div class='feature-card'>"
            f"<div class='feature-icon-wrap'>{icon_svg}</div>"
    ...
    st.markdown(
        f"<div class='start-hint'>"
        f"<span class='start-hint-icon'>{hint_svg}</span>"
        f"<span><strong>시작하기:</strong> 좌측 사이드바에서 시연 시나리오(A~E)를 선택하거나, "
        f"파라미터를 직접 설정하신 후 <em>Run simulation</em> 버튼을 누르세요.</span>"
        f"</div>",
    ...
    st.markdown(
        ...
        f"font-size: 0.78rem; color: {BRAND['text_muted']}; line-height: 1.6;'>"
        f"<strong style='color: {BRAND['text_secondary']};'>데이터 출처</strong> &nbsp;·&nbsp; "
        f"NREL ATB 2024 &nbsp;·&nbsp; IRENA RENEWCOST 2024 &nbsp;·&nbsp; BloombergNEF 2025 &nbsp;·&nbsp; "
        f"Fraunhofer ISE &nbsp;·&nbsp; METI &nbsp;·&nbsp; KEPCO &nbsp;·&nbsp; KPX &nbsp;·&nbsp; "
        f"Nature Reviews Materials (2025)<br/>"
        f"<span style='opacity: 0.7;'>{BRAND['company']}</span>"
        f"</div>",
        unsafe_allow_html=True
    )
...
def render_loading_progress(steps, current_step, label="시뮬레이션 실행 중"):
    ...
    html = (
        ...
        f"<div class='progress-title'>{label}</div>"
        f"<div class='progress-step'>Step {current_step + 1} of {len(steps)} &nbsp;·&nbsp; {current_label}</div>"
...
def render_section_title(title, subtitle=None):
    ...
    sub = (
        f"<div style='font-size: 0.82rem; color: {BRAND['text_muted']}; "
        f"margin-top: 0.15rem;'>{subtitle}</div>"
        if subtitle else ''
    )
    st.markdown(
        f"<div style='margin-top: 1rem; margin-bottom: 0.8rem;'>"
        f"<div style='font-size: 1.05rem; font-weight: 600; "
        f"color: {BRAND['text_primary']}; letter-spacing: -0.01em;'>{title}</div>"
        f"{sub}"
        f"</div>",
        unsafe_allow_html=True
    )
...
def render_sidebar_brand():
    ...
    st.sidebar.markdown(
        f"<div style='padding: 0.5rem 0 1rem 0; margin-bottom: 0.5rem; "
        f"border-bottom: 1px solid {BRAND['border']};'>"
        f"<div style='font-size: 0.95rem; font-weight: 600; "
        f"color: {BRAND['text_primary']};'>Configuration</div>"
        f"<div style='font-size: 0.75rem; color: {BRAND['text_muted']}; "
        f"margin-top: 0.1rem;'>Set parameters and run simulation</div>"
        f"</div>",
        unsafe_allow_html=True
    )
